...
HRESULT CUI_HPBAR::Initialize_Prototype(OBJECT_ID eID, _uint eUIType)
{
...
HRESULT CUI_HPBAR::Initialize_Clone(void * pArg)
{
	...
	{
		m_UIDesc.Size = _float4((_float)270.f, (_float)70.f, 0.f, 1.f);
		m_UIDesc.Position = _float4(-355.f, 330.f, 800.f, 0.98f);
...
_uint CUI_HPBAR::Tick(_double TimeDelta)
{
	return __super::Tick(TimeDelta);
}

void CUI_HPBAR::Late_Tick(_double TimeDelta)
{
...
HRESULT CUI_HPBAR::Setup_Component(void)
{
	ENGINE_OPEN;
	/* For.Com_Renderer */
	...
	/* For.Com_Shader */
	if (FAILED(__super::Add_Component(ENGINE->Get_StaticLevelIndex(), TEXT("Prototype_Component_Shader_HPBAR"), TEXT("Com_Shader"),
	...
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_PlayerHPBar"), TEXT("Com_Texture"),
...
CUI_HPBAR * CUI_HPBAR::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, OBJECT_ID eID, UITYPE eType)
{
...
CGameObject * CUI_HPBAR::Clone(void * pArg)
{
	...
	{
		MSG_BOX("Failed to Crate Clone : CUI_HPBAR");
		Safe_Release(pInstance);
		return nullptr;
	}
	return pInstance;
