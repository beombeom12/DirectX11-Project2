#include "..\public\VIBuffer_Point_Dust.h"

CVIBuffer_Point_Dust::CVIBuffer_Point_Dust(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	:CVIBuffer_Instancing(pDevice, pContext)	
{
}

CVIBuffer_Point_Dust::CVIBuffer_Point_Dust(const CVIBuffer_Point_Dust & rhs)
	: CVIBuffer_Instancing(rhs)
	, m_pData(rhs.m_pData)
{
}

HRESULT CVIBuffer_Point_Dust::Initialize_Prototype()
{
...
HRESULT CVIBuffer_Point_Dust::Initialize(void * pArg)
{
	if (pArg == nullptr)
		return E_FAIL;
	ZeroMemory(&m_DustDesc, sizeof(DUST));
	memcpy(&m_DustDesc, pArg, sizeof(DUST));
...
#pragma region VERTEX_BUFFER
	...
	pVertices->vPosition = _float3(0.0f, 3.0f, 0.0f);
...
HRESULT CVIBuffer_Point_Dust::Tick(_double TimeDelta)
{
	...
	for (_uint i = 0; i < m_iNumInstance; ++i)
	{
		...
		{
			vPos = XMLoadFloat4(p) - XMVectorSetX(XMLoadFloat3(&m_pData[i].vDirection), 0.f) * (m_pData[i].fSpeed * 0.5f) * TimeDelta;
...
HRESULT CVIBuffer_Point_Dust::Render()
{
...
void CVIBuffer_Point_Dust::Reset(_bool bSpeedChange)
{
	D3D11_MAPPED_SUBRESOURCE SubResource;
	ZeroMemory(&SubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
...
CVIBuffer_Point_Dust * CVIBuffer_Point_Dust::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CVIBuffer_Point_Dust*		pInstance = new CVIBuffer_Point_Dust(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CVIBuffer_Point_Dust");
...
CComponent * CVIBuffer_Point_Dust::Clone(void * pArg)
{
	CVIBuffer_Point_Dust*		pInstance = new CVIBuffer_Point_Dust(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Create  Clone: CVIBuffer_Point_Dust");
		Safe_Release(pInstance);
	}
...
void CVIBuffer_Point_Dust::Free()
{
	__super::Free();
